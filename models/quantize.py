...
try:
    from models.GPTQ import GenericGPTQRunner, InputRecorder, lm_eval, MultiInput
...
class QuantHandler:
    ...
    def convert_for_runtime_on_the_fly(self) -> "nn.Module":
        pass
...
def replace_linear_weight_only_int8_per_channel_on_the_fly(module):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            int8_weight, scales, _ = dynamically_quantize_per_channel(
                child.weight.float(), -128, 127, torch.int8
            )
            orig_module = getattr(module, name)
            setattr(
                module,
                name,
                WeightOnlyInt8Linear(child.in_features, child.out_features),
            )
            getattr(module, name).load_state_dict(
                {"weight": int8_weight, "scales": scales}
            )

            for _, hook in orig_module._forward_hooks.items():
                getattr(module, name).register_forward_hook(hook)

            for _, hook in orig_module._forward_pre_hooks.items():
                getattr(module, name).register_forward_pre_hook(hook)
        else:
            replace_linear_weight_only_int8_per_channel_on_the_fly(child)
...
class WeightOnlyInt8QuantHandler:
    ...
    def convert_for_runtime_on_the_fly(self):
        replace_linear_weight_only_int8_per_channel_on_the_fly(self.mod)
        return self.mod
